...
pub fn linkage_clustering_of_clusters(
    ...
    if clusters.len() > 1 {
        ...
        let jaccard_distances = Arc::new(Mutex::new(vec![
            0.;
            (clusters.len().pow(2) - clusters.len())
                / 2
        ]));
        ...
        (0..total_clusters)
            ...
            .for_each(|indices| {
                ...
                (0..clust1_size).into_par_iter().for_each(|index1| {
                    ...
                    (0..clust2_size).into_par_iter().for_each(|index2| {
                        ...
                        if assignment1.index != assignment2.index {
                            ...
                            let read_intersection: HashSet<_> = set1.intersection(&set2).collect();

                            // Extend each cluster read set
                            if read_intersection.len() == 0 {
                                let mut clust1_index = clust1_index.lock().unwrap();
                                if !clust1_index.contains(&assignment1.index) {
                                    let mut clust1_set = clust1_set.lock().unwrap();
                                    clust1_set.par_extend(set1.clone());

                                    clust1_index.insert(assignment1.index);
                                };
                                let mut clust2_index = clust2_index.lock().unwrap();
                                if !clust2_index.contains(&assignment2.index) {
                                    let mut clust2_set = clust2_set.lock().unwrap();
                ...
                let intersection: HashSet<_> = clust1_set.intersection(&clust2_set).collect();

                let mut clusters_shared_reads = clusters_shared_reads.lock().unwrap();
                let cluster_map = clusters_shared_reads
                    .entry(indices[0])
                ...
                if jaccard_distances.len() > 1 {
                    ...
                    let clusters = clusters.lock().unwrap();
                    let j_index = get_condensed_index(indices[0], indices[1], clusters.len())
                        .unwrap_or_else(|| 0);
                    let dist = 1. - jaccard;
                    jaccard_distances[j_index] = dist;
                } else {
        ...
        let dend = linkage(&mut jaccard_distances, clusters.len(), Method::Single);

        debug!(
            "Dendogram {:?} {:?} {:?}",
            &dend, jaccard_distances, clusters_shared_reads
        );
        ...
        if *changed > 0 {
            let (clusters_changed, clusters_shared_reads, jaccard_distances) =
                linkage_clustering_of_clusters(&clusters_changed, variant_info, variant_map);
            return (
                clusters_changed,
                clusters_shared_reads,
                jaccard_distances.to_vec(),
            );
        } else {
            return (
                clusters.clone(),
