...
lazy_static! {
    static ref DEFAULT_PCR_SNV_ERROR_QUAL: u8 = QualityUtils::get_phred_score_from_obs_and_errors(DEFAULT_PCR_SNV_ERROR_RATE);
...
pub fn adjust_quals_of_overlapping_paired_fragments(
    ...
    if first_read.get_end() < second_read.get_start() || first_read.get_contig() != second_read.get_contig() {
        return (first_read, second_read)
    }

    // the offset and cigar operator in the first read at the start of the left read
    let (offset, operator) = ReadUtils::get_read_index_for_reference_coordinate(first_read.get_soft_start().unwrap(), first_read.read.cigar(), second_read.get_start());
    // let operator = offset_and_operator.1;
    if offset.is_none() || match &operator {
    ...
    let first_read_end_base = ReadUtils::get_read_index_for_reference_coordinate(first_read.get_soft_start().unwrap(), first_read.read.cigar(), first_read.get_end()).0.unwrap();
    let second_read_end_base = ReadUtils::get_read_index_for_reference_coordinate(second_read.get_soft_start().unwrap(), second_read.read.cigar(), second_read.get_end()).0.unwrap();
    ...
    let second_offset = ReadUtils::get_read_index_for_reference_coordinate(second_read.get_soft_start().unwrap(), second_read.read.cigar(), second_read.get_start()).0.unwrap(); //This operation handles softclipped bases in the qual/base array
    let num_overlapping_bases = min(first_read_end_base.saturating_sub(first_read_stop), second_read_end_base.saturating_sub(second_offset)) + 1; // Add 1 here because if R1 ends on the same base that R2 starts then there is 1 base of overlap not 0
    ...
    for i in 0..num_overlapping_bases {
        ...
        if first_read_base == second_read_base {
            first_read_quals[first_read_index] = min(first_read_quals[first_read_index], half_of_pcr_error_qual);
            second_read_quals[second_read_index] = min(second_read_quals[second_read_index], half_of_pcr_error_qual);
        } else {
    ...
    first_read.update(first_read_name.as_slice(), Some(&first_read_cigar), first_read.bases.clone(), first_read_quals.as_slice());

    let second_read_cigar = second_read.read.cigar().take();
    let second_read_name = second_read.read.qname().to_vec();
    second_read.update(second_read_name.as_slice(), Some(&second_read_cigar), second_read.bases.clone(), second_read_quals.as_slice());
