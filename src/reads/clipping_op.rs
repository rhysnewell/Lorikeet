...
use rust_htslib::bam::record::{CigarString, Cigar, CigarStringView};
...
impl ClippingOp {
     ...
     */
    pub fn apply(&self, algorithm: ClippingRepresentation, original_read: &BirdToolRead) -> BirdToolRead {
        match algorithm {
    ...
    fn apply_revert_soft_clipped_bases(&self, read: &BirdToolRead) -> BirdToolRead {
        ...
        unclipped.read.set(read.read.qname(), Some(&unclipped_cigar), read.read.seq().encoded, read.read.qual());
    ...
    fn apply_write_q0s(read_copied: &mut BirdToolRead, original_read: &BirdToolRead) {
        let mut new_quals = read_copied.read.qual().clone();
        Self::overwrite_from_start_to_stop(&mut new_quals, 0 as u8);
        read_copied.read.set(original_read.read.qname(), Some(&original_read.read.cigar().take()), original_read.read.seq().encoded, &new_quals);
    }

    fn apply_write_ns(read_copied: &mut BirdToolRead, original_read: &BirdToolRead) {
        let mut new_bases = read_copied.read.seq().encoded.clone();
        Self::overwrite_from_start_to_stop(&mut new_bases, 'N'.as_());
        read_copied.read.set(original_read.read.qname(), Some(&original_read.read.cigar().take()), &new_bases, original_read.read.qual());
    ...
    fn apply_write_ns_and_q0s(read_copied: &mut BirdToolRead, original_read: &BirdToolRead) {
        ...
        read_copied.read.set(original_read.read.qname(), Some(&original_read.read.cigar().take()), &new_bases, &new_quals);
    }

    fn overwrite_from_start_to_stop(arr: &mut [u8], new_val: u8) {
        arr.par_iter().for_each(|val| *val = new_val);
    }
    ...
    fn apply_hard_clip_bases(read: &BirdToolRead, start: usize, stop: usize) -> BirdToolRead {
        ...
        let new_cigar = if read.read.is_unmapped() { CigarString(vec![Cigar::Match(0)]) } else { CigarUtils::clip_cigar(&cigar, start as u32, (stop + 1) as u32, Cigar::HardClip(0)) };
