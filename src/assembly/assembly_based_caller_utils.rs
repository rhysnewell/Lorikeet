...
impl AssemblyBasedCallerUtils {
    ...
    pub fn finalize_regions(
        ...
        if !region.is_finalized() {
            let min_tail_quality_to_use = if error_correct_reads { HaplotypeCallerEngine::MIN_TAIL_QUALITY_WITH_ERROR_CORRECTION as u8 } else { min_tail_quality };
            ...
            for original_read in region.get_reads().iter() {
                ...
                let mut read = if dont_use_soft_clipped_bases || !ReadUtils::has_well_defined_fragment_size(&original_read) {
                ...
                if read.get_start() <= read.get_end() {
                    ...
                    if !read.read.is_empty() && read.read.seq_len_from_cigar(false) > 0 {
                        read = ReadClipper::hard_clip_to_region(read, region.get_padded_span().get_start(), region.get_padded_span().get_end());

                        if read.get_start() <= read.get_end() && read.read.len() > 0 && read.overlaps(region.get_padded_span()) {
    ...
    pub fn assemble_reads<'a, C: ChainPruner<MultiDeBruijnVertex<'a>, MultiSampleEdge>>(
        ...
        Self::finalize_regions(
            ...
            args.value_of("base-quality-threshold").unwrap().parse::<u8>().unwrap() - 1,
            correct_overlapping_base_qualities,
            args.is_present("soft-clip-low-quality-ends")
        );
        debug!("Assembling {:?} with {} reads:    (with overlap region = {:?})", region.get_span(), region.get_reads().len(), region.get_padded_span());

        let full_reference_with_padding = region.get_assembly_region_reference(reference_reader, Self::REFERENCE_PADDING_FOR_ASSEMBLY);
        let padded_reference_loc = Self::get_padded_reference_loc(&region, Self::REFERENCE_PADDING_FOR_ASSEMBLY, &reference_reader);
        let ref_haplotype = Self::create_reference_haplotype(&region, &padded_reference_loc, reference_reader);
    ...
    pub fn get_padded_reference_loc(
        region: &AssemblyRegion, reference_padding: usize, reference_reader: &ReferenceReader<'_>
    ) -> SimpleInterval {
        let pad_left = max(region.get_padded_span().get_start().checked_sub(reference_padding).unwrap_or(0), 1);
        let pad_right = min(region.get_padded_span().get_end() + reference_padding, reference_reader.get_contig_length(region.get_contig()) as usize);
